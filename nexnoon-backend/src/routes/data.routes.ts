...
router.delete(
  ...
  async (req: AuthRequest, res) => {
    ...
    await ClassModel.updateOne(
      { _id: enrollment.classId, enrolledStudents: { $gt: 0 } },
 ...
 */
router.get('/learner', requireAuth, requireRole('student', 'admin'), async (req: AuthRequest, res) => {
  const userId = req.user!.id;
  const viewerTz = (await User.findById(userId).select('timezone').lean())?.timezone;
  ...
  const submittedKeys = new Set(
    submissions.map((s) => `${String(s.classId)}:${String(s.assignmentId)}`)
  );
  const now = Date.now();
  ...
  const enrolled: ClassCard[] = [];
  const upcoming: ClassCard[] = [];
  const completed: ClassCard[] = [];
  const continueLearning: ClassCard[] = [];
  const certificates: {
    classId: string;
    title: string;
    instructor: string;
  ...
  for (const e of enrollments) {
    ...
    const classSessions = sessions.filter((s) => String(s.classId) === String(cls._id));
    const next = classSessions.find(
      (s) => s.status === 'live' || new Date(s.endTime).getTime() > now
    );
    const nextLive = next?.status === 'live';

    // Hours from attended session durations (fallback: progress × class duration hours)
    const attendedIds = new Set((e.attendedSessions || []).map((id) => String(id)));
    let attendedMs = 0;
    for (const s of classSessions) {
      if (attendedIds.has(String(s._id))) {
        attendedMs += Math.max(0, new Date(s.endTime).getTime() - new Date(s.startTime).getTime());
      }
    }
    if (attendedMs > 0) {
      hoursLearned += attendedMs / 3600000;
    } else if ((e.progress || 0) > 0 && cls.duration) {
      hoursLearned += ((e.progress || 0) / 100) * (cls.duration || 0);
    }

    const assignments = (cls.assignments as any[]) || [];
    let dueCount = 0;
    for (const a of assignments) {
      const assignmentId = String(a._id);
      const submitted = submittedKeys.has(`${String(cls._id)}:${assignmentId}`);
      const due = a.dueDate ? new Date(a.dueDate).getTime() : null;
      const open = !submitted && (due == null || due >= now - 7 * 86400000);
      if (!submitted && (due == null || due >= now)) dueCount += 1;
      if (open || (!submitted && due != null && due < now)) {
        dueAssignments.push({
          id: assignmentId,
    ...
    const card: ClassCard = {
      ...
      progress: e.progress ?? 0,
      status: e.status,
      nextSession: next ? formatWhen(next.startTime, pickTimeZone(viewerTz, cls.timezone)) : null,
      nextSessionStart: next ? new Date(next.startTime).toISOString() : null,
      nextSessionId: next ? String(next._id) : null,
      nextSessionLive: !!nextLive,
      assignmentCount: assignments.length,
      dueAssignmentCount: dueCount,
      certificateUrl: e.certificateUrl || null,
      completedAt: e.completedAt ? new Date(e.completedAt).toISOString() : null,
      enrolledAt: new Date(e.enrolledAt).toISOString(),
      startDate: cls.startDate ? new Date(cls.startDate).toISOString() : null,
    };
    ...
    if (e.status === 'completed') {
      ...
      const classStart = cls.startDate ? new Date(cls.startDate).getTime() : 0;
      if (classStart > now && !next) {
        upcoming.push(card);
      } else {
        enrolled.push(card);
        continueLearning.push(card);
      }
    }
  }

  continueLearning.sort((a, b) => {
    if (a.nextSessionLive !== b.nextSessionLive) return a.nextSessionLive ? -1 : 1;
    const at = a.nextSessionStart ? new Date(a.nextSessionStart).getTime() : Infinity;
    const bt = b.nextSessionStart ? new Date(b.nextSessionStart).getTime() : Infinity;
    return at - bt;
  });

  const upcomingSessions = sessions
    .filter((s) => s.status === 'live' || new Date(s.endTime).getTime() > now)
    .slice(0, 12)
    .map((s) => {
      const cls = classesById.get(String(s.classId));
      return {
  ...
  const openDue = dueAssignments.filter((a) => !a.submitted);
  const nextSessionAt = upcomingSessions[0]?.startTime || null;

  return res.json({
    success: true,
    data: {
      summary: {
        inProgress: enrolled.length,
        upcoming: upcoming.length,
        completed: completed.length,
        certificates: certificates.length,
        assignmentsDue: openDue.length,
        hoursLearned: Math.round(hoursLearned * 10) / 10,
        nextSessionAt,
      },
      continueLearning: continueLearning.slice(0, 6),
      upcomingSessions,
      dueAssignments: openDue.slice(0, 10),
      certificates,
      enrolled,
      upcoming,
      completed,
