...
async function main() {
  ...
  let accounts = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8')) : [
    { email: 'admin.demo@nexnoon.test', role: 'admin', firstName: 'Demo', lastName: 'Admin', password: 'Admin-' + crypto.randomBytes(6).toString('hex') + '!' },
  ...
  // Ensure platform seat settings exist for local demos.
  const { PlatformSettingsModel, PLATFORM_SETTINGS_KEY, DEFAULT_MAX_CLASS_SEATS } = require('../dist/models/PlatformSettings');
  await PlatformSettingsModel.findOneAndUpdate(
    { key: PLATFORM_SETTINGS_KEY },
    { $setOnInsert: { key: PLATFORM_SETTINGS_KEY, maxClassSeats: DEFAULT_MAX_CLASS_SEATS } },
    { upsert: true }
  );
  ...
  for (const account of accounts) {
    if (!(await User.exists({ email: account.email }))) {
      await User.create({
        ...
        instructorStatus: account.role === 'instructor' ? 'approved' : account.role === 'admin' ? 'none' : 'none',
      });
    } else if (account.role === 'instructor') {
      await User.updateOne({ email: account.email }, { $set: { instructorStatus: 'approved' } });
    }
  }
  ...
  const teacherAccount = accounts.find(a => a.role === 'instructor');
  const studentAccount = accounts.find(a => a.role === 'student');
  const teacher = await request('/auth/login', null, teacherAccount);
  const student = await request('/auth/login', null, studentAccount);
